        ...
        if (!empty($unique_fields)) {
            ...
            foreach ($add_fields as $add_field) {
                ...
                if (in_array($field_name, $unique_fields)) {
                    $form_validation->set_rules($field_name, $field_types[$field_name]->display_as, 'is_unique[' . $this->basic_db_table . '.' . $field_name . ']');
                }
            }
        ...
        if ($upload && is_array($upload['tmp_name'])) {
            ...
            $info[] = $this->handle_file_upload(
                    isset($upload['tmp_name']) ? $upload['tmp_name'] : null, isset($_SERVER['HTTP_X_FILE_NAME']) ?
                            ...
                            $_SERVER['HTTP_X_FILE_TYPE'] : $upload['type'], isset($upload['error']) ? $upload['error'] : null
            );
        }
        header('Vary: Accept');
